...
<div class="max-w-lg mx-auto">
    <h1 class="text-2xl font-bold text-gray-900 mb-6">Transfer Antar Wallet</h1>

    <div class="bg-white rounded-lg shadow p-6">
        <form method="POST" action="{{ route('transactions.storeTransfer') }}">
            @csrf
            <div class="space-y-4">
                <div>
                    <label for="from_account_id" class="block text-sm font-medium text-gray-700 mb-1">Dari Wallet</label>
                    <select name="from_account_id" id="from_account_id"
                        ...
                        @foreach($wallets as $wallet)
                            <option value="{{ $wallet->id }}" {{ old('from_account_id') == $wallet->id ? 'selected' : '' }}>
                                {{ $wallet->name }} (Rp {{ number_format($wallet->balance, 0, ',', '.') }})
                            </option>
                        @endforeach
                    </select>
                    @error('from_account_id')<p class="text-red-500 text-sm mt-1">{{ $message }}</p>@enderror
                </div>

                <div>
                    <label for="to_account_id" class="block text-sm font-medium text-gray-700 mb-1">Ke Wallet</label>
                    <select name="to_account_id" id="to_account_id"
                            class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                            required>
                        <option value="">-- Pilih Wallet Tujuan --</option>
                        @foreach($wallets as $wallet)
                            <option value="{{ $wallet->id }}" {{ old('to_account_id') == $wallet->id ? 'selected' : '' }}>
                                {{ $wallet->name }} (Rp {{ number_format($wallet->balance, 0, ',', '.') }})
                            </option>
                        @endforeach
                    </select>
                    @error('to_account_id')<p class="text-red-500 text-sm mt-1">{{ $message }}</p>@enderror
                </div>
            ...
            <div class="flex gap-3 mt-6">
                ...
                <button type="submit" class="flex-1 bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 font-medium">Transfer</button>
            </div>
        </form>
    </div>
</div>
